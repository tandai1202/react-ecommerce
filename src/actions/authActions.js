...
export const makeAuthRequest = async(dispatch, loginData) => {
    ...
    try {
        ...
        if(!error?.response) {
            dispatch({ type: actionType.AUTH_ERROR, payload: "No sever response" });
        }

        if(error.response?.status === 400) {
            dispatch({ type: actionType.AUTH_ERROR, payload: "Mising username or password" });
        }

        if(error.response?.status === 401) {
            dispatch({ type: actionType.AUTH_ERROR, payload: "Unauthorized!" });
        }
    }
}
